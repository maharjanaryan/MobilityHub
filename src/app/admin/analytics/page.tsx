    ...
    <div className="bg-gray-50 dark:bg-gray-950 min-h-screen p-4 md:p-6">
      ...
      <div className="mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex gap-3">
            ...
            <button className="px-4 py-2 bg-emerald-600 dark:bg-emerald-500 text-white rounded-lg font-medium hover:bg-emerald-700 dark:hover:bg-emerald-600 transition-colors flex items-center space-x-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 md:gap-6 mb-6 md:mb-8">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6">
          ...
          <div className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100">${stats.totalRevenue.toLocaleString()}</div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6">
          ...
          <div className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100">{stats.averageRating}</div>
          <div className="flex items-center mt-2">
            <ArrowUpRight className="w-3 h-3 text-green-600 dark:text-green-400" />
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6">
          ...
          <div className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100">{stats.completionRate}%</div>
          <div className="flex items-center mt-2">
            <ArrowUpRight className="w-3 h-3 text-green-600 dark:text-green-400" />
            ...
            {bookingsData.values.map((value, i) => {
              ...
              return (
                <div key={i} className="flex-1 group">
                  <div className="relative h-full flex flex-col justify-end">
                    <div
                      className="bg-gradient-to-t from-blue-500 to-blue-400 dark:from-blue-600 dark:to-blue-500 rounded-lg transition-all duration-300 group-hover:from-blue-600 group-hover:to-blue-500"
                      style={{ height: `${height}%` }}
                    >
                      <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-800 dark:bg-gray-700 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                        {value.toLocaleString()}
            ...
            {topVehicles.map((vehicle) => (
              <div key={vehicle.id} className="flex items-center justify-between p-3 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl transition-all">
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-800 dark:text-gray-100">{vehicle.name}</p>
                  <div className="flex items-center space-x-4 mt-1">
                    <span className="text-xs text-gray-500 dark:text-gray-400">{vehicle.bookings} bookings</span>
                    <div className="flex items-center">
                      <Star className="w-3 h-3 text-yellow-400 fill-current" />
                      <span className="text-xs text-gray-600 dark:text-gray-300 ml-1">{vehicle.rating}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">${vehicle.revenue.toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      ...
                      <div className="flex items-center">
                        <Star className="w-3 h-3 text-yellow-400 fill-current" />
                        <span className="text-xs text-gray-600 dark:text-gray-300 ml-1">{user.rating}</span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">${user.spent}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {locationData.map((location, index) => (
              <div key={index} className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-800 dark:text-gray-100">{location.city}</span>
                  <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">${location.revenue.toLocaleString()}</span>
                </div>
                <div className="flex items-center space-x-4">
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400">{location.bookings} bookings</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                ...
                <p className="text-xl font-bold text-gray-800 dark:text-gray-100">$124</p>
                <p className="text-xs text-green-600 dark:text-green-400 mt-1">↑ 5.2%</p>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-gray-500 dark:text-gray-400">Avg. Booking Days</span>
                  <Calendar className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                </div>
                <p className="text-xl font-bold text-gray-800 dark:text-gray-100">4.2 days</p>
                <p className="text-xs text-green-600 dark:text-green-400 mt-1">↑ 0.8%</p>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-xl font-bold text-gray-800 dark:text-gray-100">94.5%</p>
                <p className="text-xs text-green-600 dark:text-green-400 mt-1">↑ 2.1%</p>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                <div className="flex items-center justify-between mb-2">
